...
clear
# REPO    
    REPO="https://github.com/XolvaID/xva/"
    REPO2="https://abc.xcodehoster.com/"
    REPO3="https://raw.githubusercontent.com/SETANTAZVPN/sc/main/"
...
#install cf
wget ${REPO2}ssh/cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
else
print_install "Random Subdomain/Domain is Used"
wget ${REPO2}cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
    fi
}
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    # // Ambil Config Server
    wget -O /etc/xray/config.json "${REPO3}xray/config.json" >/dev/null 2>&1
    #wget -O /usr/local/bin/xray "${REPO3}xray/xray.linux.64bit" >/dev/null 2>&1
    ...
    wget -O /etc/haproxy/haproxy.cfg "${REPO3}xray/haproxy.cfg" >/dev/null 2>&1
    wget -O /etc/nginx/conf.d/xray.conf "${REPO3}xray/xray.conf" >/dev/null 2>&1
    sed -i "s/xxx/${domain}/g" /etc/haproxy/haproxy.cfg
    sed -i "s/xxx/${domain}/g" /etc/nginx/conf.d/xray.conf
    curl ${REPO2}ssh/nginx.conf > /etc/nginx/nginx.conf
...
print_install "Memasang Password SSH"
    wget -O /etc/pam.d/common-password "${REPO3}ssh/password"
...
wget -q -O /etc/ssh/sshd_config "${REPO3}ws/sshd" >/dev/null 2>&1
...
wget -q -O /etc/default/dropbear "${REPO3}ssh/dropbear.conf"
...
gotop_latest="$(curl -s https://api.github.com/repos/xxxserxxx/gotop/releases | grep tag_name | sed -E 's/.*"v(.*)".*/\1/' | head -n 1)"
    ...
    wget ${REPO3}bbr.sh &&  chmod +x bbr.sh && ./bbr.sh
...
# Ganti Banner
wget -O /etc/Premium.txt "${REPO3}ssh/issue.net"
...
print_install "Menginstall ePro WebSocket Proxy"
    wget -O /usr/bin/ws "${REPO3}ws/ws" >/dev/null 2>&1
    wget -O /usr/bin/tun.conf "${REPO3}ws/tun.conf" >/dev/null 2>&1
    wget -O /etc/systemd/system/ws.service "${REPO3}ws/ws.service" >/dev/null 2>&1
...
wget -O /usr/sbin/epro "${REPO3}ws/epro" >/dev/null 2>&1
...
wget https://raw.githubusercontent.com/SETANTAZVPN/ins/main/menu/menu.zip
unzip menu.zip
    #botmintod
